    ...
    if ( tmp_dtm < 0 )
      ...
      % Valuation for: European plain vanilla options
        if ( strcmpi(option_type,'European')  )     % calling Black-Scholes option pricing model
			...
			% theo_value	= pricing_option_cpp(1,logical(call_flag),tmp_underlying_value, ...
                                % tmp_strike,tmp_dtm_pricing,tmp_rf_rate_conv, ...
                                % tmp_imp_vola_shock,divyield);
			% theo_value = theo_value.* tmp_multiplier;
      ...
      % Valuation for: American plain vanilla options
	   ...
	   % Valuation for: European Binary Options:
        elseif ( strcmpi(option_type,'Binary'))   % calling Binary option pricing model
            theo_value	= option_binary(call_flag, obj.binary_type, tmp_underlying_value, ...
                            tmp_strike, obj.payoff_strike, tmp_dtm_pricing, tmp_rf_rate_conv, ...
                            tmp_imp_vola_shock, divyield) .* tmp_multiplier;
			
	   % Valuation for: European Lookback Options:
        elseif ( strcmpi(option_type,'Lookback'))   % calling Lookback option pricing model
             theo_value	= option_lookback(call_flag, obj.lookback_type, tmp_underlying_value, ...
                            tmp_strike, obj.payoff_strike, tmp_dtm_pricing, tmp_rf_rate_conv, ...
                            tmp_imp_vola_shock, divyield) .* tmp_multiplier;
		end						
