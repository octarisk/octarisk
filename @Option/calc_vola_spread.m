        ...
        elseif ( strcmpi(option_type,'American'))
			...
			elseif ( strcmpi(obj.pricing_function_american,'CRR') )
                ...
                tmp_optionvalue_base	= pricing_option_cpp(2,logical(call_flag),tmp_underlying_value_base, ...
                                    tmp_strike,tmp_dtm_pricing,tmp_rf_rate_base, ...
                                    tmp_indexvol_base,divyield,treenodes);
				tmp_optionvalue_base = tmp_optionvalue_base .* tmp_multiplier;
				
				% set up objective function		
				objfunc = @ (x) phi_crr (x,2,logical(call_flag),tmp_underlying_value_base, ...
                                    tmp_strike,tmp_dtm_pricing,tmp_rf_rate_base, ...
                                    tmp_indexvol_base,divyield,treenodes,tmp_multiplier,tmp_value);
            ...
            elseif (  strcmpi(option_type,'American'))   
                if ( strcmpi(obj.pricing_function_american,'Willowtree') )
                    tmp_new_val = option_willowtree(call_flag,1, ...
                                ...
                                obj.willowtree_nodes,path_static) .* tmp_multiplier;
				elseif ( strcmpi(obj.pricing_function_american,'CRR') )
					treenodes 	= round(tmp_dtm_pricing/option.timesteps_size);
					...
					tmp_new_val = tmp_new_val .* tmp_multiplier;
                else
