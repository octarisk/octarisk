function obj = calc_vola_spread(option,underlying,vola_riskfactor,discount_curve,tmp_vola_surf_obj,valuation_date,path_static)
    ...
    if ( tmp_dtm < 0 )
        ...
        if ( strfind(tmp_type,'OPT_EUR') > 0 )
            tmp_optionvalue_base        = option_bs(call_flag,tmp_underlying_value_base,tmp_strike,tmp_dtm,tmp_rf_rate_base,tmp_indexvol_base) .* tmp_multiplier;
            tmp_impl_vola_spread        = calibrate_option_bs(call_flag,tmp_underlying_value_base,tmp_strike,tmp_dtm,tmp_rf_rate_base,tmp_indexvol_base,tmp_multiplier,tmp_value);
        elseif ( strfind(tmp_type,'OPT_AM') > 0 )
            tmp_optionvalue_base        = option_willowtree(call_flag,1,tmp_underlying_value_base,tmp_strike,tmp_dtm,tmp_rf_rate_base,tmp_indexvol_base,0.0,option.timesteps_size,option.willowtree_nodes,path_static) .* tmp_multiplier;
            tmp_impl_vola_spread        = calibrate_option_willowtree(call_flag,1,tmp_underlying_value_base,tmp_strike,tmp_dtm,tmp_rf_rate_base,tmp_indexvol_base,0.0,option.timesteps_size,option.willowtree_nodes,tmp_multiplier,tmp_value,path_static);
        end
        ...
        if ( tmp_impl_vola_spread < -98 )
            ...
            %tmp_value
            if ( strfind(tmp_type,'OPT_EUR') > 0  )
                tmp_new_val             = option_bs(call_flag,tmp_underlying_value_base,tmp_strike,tmp_dtm,tmp_rf_rate_base,tmp_indexvol_base + tmp_impl_vola_spread) .* tmp_multiplier;
            elseif ( strfind(tmp_type,'OPT_AM') > 0 )
                tmp_new_val             = option_willowtree(call_flag,1,tmp_underlying_value_base,tmp_strike,tmp_dtm,tmp_rf_rate_base,tmp_indexvol_base + tmp_impl_vola_spread,0.0,option.timesteps_size,option.willowtree_nodes,path_static) .* tmp_multiplier;
            end
