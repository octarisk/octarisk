...
% valuation for 'SENSI' instruments (linear and quadratic combination of underlyings)
  ...
  if ( strcmpi(scenario,'base') &&  sensi.use_value_base == true )
    ...
    % input check
    if ~( length(sensi_prefactor) == length(sensi_exponent)) ...
        || ~( length(sensi_prefactor) == length(sensi_cross))  ...
        || ~( length(sensi_prefactor) == length(x_coord)) ...
        || ~( length(sensi_prefactor) == length(y_coord)) ...
        || ~( length(sensi_prefactor) == length(z_coord)) ...
        || ~( length(sensi_prefactor) == length(shock_type)) ...
        || ~( length(sensi_prefactor) == length(underlyings))
