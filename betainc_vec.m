...
## -*- texinfo -*-
## @deftypefn {} {} betainc_vec (@var{x}, @var{a}, @var{b})
## @deftypefnx {} {} betainc_vec (@var{x}, @var{a}, @var{b}, @var{tail})
## Compute the incomplete beta function ratio.
##
## This function shadows the core function betainc. Performance improvement
## due to vectorized C++. Support for single precision was removed.
##
## This is defined as
## @tex
## $$
...
function [y] = betainc_vec (x, a, b, tail = "lower")
...
%!test #<51157>
%! y = betainc_vec([0.00780;0.00782;0.00784],250.005,49750.995);
%! y_ex = [0.999999999999989; 0.999999999999992; 0.999999999999995];
%! assert (y, y_ex, -1e-14);

%!assert (betainc_vec (0.001, 20, 30), 2.750687665855991e-47, -3e-14);
%!assert (betainc_vec (0.0001, 20, 30), 2.819953178893307e-67, -3e-14);
%!assert (betainc_vec (0.99, 20, 30, "upper"), 1.5671643161872703e-47, -3e-14);
%!assert (betainc_vec (0.999, 20, 30, "upper"), 1.850806276141535e-77, -3e-14);
%!assert (betainc_vec (0.5, 200, 300), 0.9999964565197356, -1e-15);
%!assert (betainc_vec (0.5, 200, 300, "upper"), 3.54348026439253e-06, -1e-13);

# Test trivial values

%!test
%! [a,b] = ndgrid (linspace(1e-4, 100, 20), linspace(1e-4, 100, 20));
%! assert (betainc_vec (0,a,b), zeros(20));
%! assert (betainc_vec (1,a,b), ones(20));

## Test input validation

%!error betainc_vec ()
%!error betainc_vec (1)
%!error betainc_vec (1,2)
%!error betainc_vec (1.1,1,1)
%!error betainc_vec (-0.1,1,1)
%!error betainc_vec (0.5,0,1)
%!error betainc_vec (0.5,1,0)
%!error betainc_vec (1,2,3,4)
%!error betainc_vec (1,2,3,4,5)
%!error betainc_vec (0.5i, 1, 2)
%!error betainc_vec (0, 1i, 1)
%!error betainc_vec (0, 1, 1i)
