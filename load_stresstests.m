...
%# -*- texinfo -*-
%# @deftypefn {Function File} {[@var{portfolio_struct} @var{id_failed_cell}] =} 
%# load_stresstests(@var{portfolio_struct}, @var{valuation_date}, 
%# @var{path_stresstests}, @var{file_stresstests}, @var{path_output}, 
%# @var{path_archive}, @var{tmp_timestamp}, @var{archive_flag})
...
% B) Loop through all position files -> loop through all rows 
%    (first row = header), for each row loop through all columns = 
%     attributes -> store attributes in cell
...
for ii = 1 : 1 : length(tmp_list_files)
    ...
    if (length(tmp_filename) > 3 && strcmp(tmp_filename,'comments.txt') == 0) 
        ...
        %# explicit list taken from 'man pcrepattern' -- we enclose all
          ...
          lines(1 == dummy) = {''};
          lines_out = {};
           %# remove everything beyond the eol character 
           % (the character number dummy value found)
            ...
            for mm = 2 : 1 : length(tmp_cell)   
                ...
                elseif ( strcmp(tmp_cell_type,'BOOL'))
                    try                    
                        ...
                        elseif ( ischar(tmp_cell_item))
                            if ( strcmp('false',lower(tmp_cell_item)) ...
                                                || strcmp('0',tmp_cell_item))
                                tmp_entry = 0;
                            elseif ( strcmp('true',lower(tmp_cell_item)) ...
                                                || strcmp('1',tmp_cell_item))
                                tmp_entry = 1;
                            end
                        end  
                    catch
                ...
                try
                    ...
                    elseif ( strcmp(tmp_columnname,'shifttype')) % split into cell
                        ...
                        tmp_entry(tmp_entry > 0) = 1;   % limit shift type to 
                                        % either 0 or 1 -> prevent 
                                        % problems with stress value calculation
