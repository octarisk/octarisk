...
function [surface_struct vola_failed_cell] = load_volacubes(surface_struct,path_mktdata,input_filename_vola_index,input_filename_vola_ir)
...
for ii = 1 : 1 : length(tmp_list_files)
    ...
    try % try to find a match between file in mktdata folder and provided string for INDEX and IR volatilities
        ...
        elseif ( regexp(tmp_filename,input_filename_vola_ir) == 1)       % vola for ir found  
            ...
            %fprintf('no match found for file >>%s<<. Skipping file. \n',tmp_filename);
            tmp_id = 'Nothing found';
        end 
    catch
        fprintf('WARNING: There has been an error for file: >>%s<< and vola object id: >>%s<<. Aborting: >>%s<<\n',tmp_filename,tmp_id,lasterr);
