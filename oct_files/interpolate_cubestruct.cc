...
all cube fieldnames.\n\
This function should be called from Octave Surface method getValue\n\
...
@end deftypefn")
{
  ...
  double zz            = args(3).double_value ();  // zz coordinates to interpolate

  // get fieldnames of provided struct
    std::string fieldname_cube = "cube";
    std::string fieldname_id = "id";
	std::string fieldname_xx = "x_axis";
	std::string fieldname_yy = "y_axis";
	std::string fieldname_zz = "z_axis";
  ...
  for (octave_idx_type ii = 0; ii < len; ii++) 
	{
		...
		// get nearest values for xx, yy and zz from axis values
			...
			double z0 = previous_neighbour(zz_values, zz);
			double z1 = next_neighbour(zz_values, zz);
			std::cout << "y0:" << y0 << "\n";
			std::cout << "y1:" << y1 << "\n";
		// get differences
			double xd, yd, zd;
		...
		// extract volatility value
			...
			std::cout << "V_x0y0z0:" << V_x0y0z0 << "\n";
			std::cout << "V_x0y0z1:" << V_x0y0z1 << "\n";
...
bool any_bad_argument(const octave_value_list& args)
{
	...
	if (! args(0).is_map ())
	{
		error ("interpolate_cubestruct: ARG1 must be a struct");
		return true;
    }

	if (! args(1).is_numeric_type())
	{
		error ("interpolate_cubestruct: ARG2 must be numeric (Term)");
		return true;
    }
	...
	{
		error ("interpolate_cubestruct: ARG2 must be numeric (Moneyness)");
		return true;
    }
